...
void CEncryptFileDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT2, m_encryptpath);
	DDX_Text(pDX, IDC_EDIT3, m_decryptpath);
	//}}AFX_DATA_MAP
}
...
void CEncryptFileDlg::OnEncrypt() 
{
	UpdateData();
	CAES aes;
	aes.SetKeys(BIT128,"AES Encrypt Decrypt");
	aes.EncryptFile(m_sourcepath,m_encryptpath);
	UpdateData(false);
}

void CEncryptFileDlg::OnDecrypt() 
{
	UpdateData();
	CAES aes;
	aes.SetKeys(BIT128,"AES Encrypt Decrypt");
	aes.DecryptFile(m_encryptpath,m_decryptpath);
	UpdateData(false);
}
