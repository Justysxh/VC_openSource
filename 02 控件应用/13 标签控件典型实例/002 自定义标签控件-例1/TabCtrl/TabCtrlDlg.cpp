...
BOOL CTabCtrlDlg::OnInitDialog()
{
	...
	m_IndexDlg.Create(IDD_INDEXDLG_DIALOG, &m_Tab);
	...
	m_Tab.InsertItem(0, "Ë÷Òý", 0);
	m_Tab.InsertItem(1, "²éÕÒ", 1);
	m_Tab.InsertItem(2, "Ä¿Â¼", 2);
	m_Tab.SetCurSel(0);
	LRESULT result;
...
void CTabCtrlDlg::OnSelchangeTab1(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	clientRC.DeflateRect(1, 5, 1, 1);
	clientRC.bottom -= 40;
	switch (nCurSel)
	{
	case 0:
		m_IndexDlg.MoveWindow(clientRC);
		m_IndexDlg.ShowWindow(SW_SHOW);
		break;
	case 1:
		m_IndexDlg.ShowWindow(SW_HIDE);
		Invalidate();
		break;
	case 2:
		m_IndexDlg.ShowWindow(SW_HIDE);
